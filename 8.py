...
OBSTACLES = [(2, 3), (3, 4), (5, 7)]  # Example obstacle locations
...
for episode in range(NUM_EPISODES):
    ...
    while state != GRID_SIZE * GRID_SIZE - 1:  # Continue until reaching the goal state
        ...
        if (next_state // GRID_SIZE, next_state % GRID_SIZE) not in OBSTACLES:
...
def draw_grid(agent_pos):
    ...
    for i in range(GRID_SIZE):
        for j in range(GRID_SIZE):
            color = GRAY if (i, j) in OBSTACLES else RED if (i, j) == (GRID_SIZE - 1, GRID_SIZE - 1) else BLACK
...
while running:
    ...
    if action == 0:  # UP
        next_pos = (max(0, agent_pos[0] - 1), agent_pos[1])
    elif action == 1:  # DOWN
        next_pos = (min(GRID_SIZE - 1, agent_pos[0] + 1), agent_pos[1])
    elif action == 2:  # LEFT
        next_pos = (agent_pos[0], max(0, agent_pos[1] - 1))
    elif action == 3:  # RIGHT
        next_pos = (agent_pos[0], min(GRID_SIZE - 1, agent_pos[1] + 1))
